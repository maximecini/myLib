...
/*   Updated: 2024/12/14 13:31:10 by rcini-ha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * ou par la fin du fichier (EOF).Elle gère un buffer interne pour conserver 
 * l'état de lecture entre plusieurs appels.
 *
 * @param fd Le descripteur de fichier à lire. Il peut s'agir d'un 
 * 	descripteur obtenu via `open()` ou des descripteurs standards 
 *  comme `0` (stdin).
 *
 *  @return Une chaîne de caractères contenant la ligne lue 
 * 		(incluant le caractère '\n'si présent),
	allouée dynamiquement. Si la fin du fichier est atteinte et
 ...
 *
 * @note L'utilisateur est responsable de libérer la mémoire allouée 
 * pour chaque ligneavec `free()`. La macro `BUFFER_SIZE` doit 
 * être définie pour déterminer la taille du tampon interne utilisé 
 * lors de la lecture.
 *
 * @note Cette fonction n'est pas thread-safe en raison de l'utilisation 
 * d'un buffer statique partagé entre les appels.
 *
